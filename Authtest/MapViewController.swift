...
//            clearRoute()
//            
//            mapTasks.getDirections(mapTasks.originAddress, destination: mapTasks.destinationAddress, waypoints: waypointsArray, travelMode: travelMode.self, completionHandler: { (status, success) -> Void in
//                
//                if success {
//                    self.configureMapAndMarkersForRoute()
//                    self.drawRoute()
...
class MapViewController: UIViewController, CLLocationManagerDelegate, GMSMapViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let camera: GMSCameraPosition = GMSCameraPosition.cameraWithLatitude(34.0522, longitude: -118.2437, zoom: 8.0)
        viewMap.camera = camera
